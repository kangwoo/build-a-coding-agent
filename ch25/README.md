...
책 25장의 산출물. 외부 **MCP(Model Context Protocol)** 서버에 stdio로 연결해 그 도구를 가져와, 7장 `Tool` 인터페이스로 감싸 레지스트리에 등록한다. 어댑터 하나면 12~24장 코드는 그것이 외부 도구인지 **모른다** — 외부 도구가 우리 도구와 똑같은 1급 시민이 된다. **확장성의 마지막 조각**이자 선택/심화 장이다.
