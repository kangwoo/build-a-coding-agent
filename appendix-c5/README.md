...
[`appendix/C-확장-아이디어.md` §C.5](../../appendix/C-확장-아이디어.md)의 참조 구현. 23장에서 *골격*만 짚은 백그라운드 태스크를 동작하는 형태로 채운다 — 진행 출력을 디스크에 흘리며 **오프셋으로 증분 폴링**하는 저장소(`BackgroundTasks`)와, 그 위에 얹는 **`TaskOutput`·`TaskStop`** 도구다.
