...
책 7장의 산출물. 앞으로 만들 모든 도구가 공통으로 따를 **`Tool<I,O>` 계약**(필수 메서드 + 안전한 `default` 기본값)과, `tool_use` 한 개를 받아 `tool_result` 한 개로 만드는 **단일 실행 파이프라인**(`ToolExecutor`)을 만든다. 더미 도구 `EchoTool`로 계약이 도는지 확인한다.
