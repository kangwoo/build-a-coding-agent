...
public interface Tool<I, O> {
    ...
    /** 권한 규칙(예: "Bash(npm install)") 매칭에 쓸 문자열. 도구마다 의미가 다르다(16장).
     *  경로 기반 도구는 실행(call)과 같은 기준(ctx.workingDir())으로 해석해야 규칙이 실제 대상과 일치한다. */
    default java.util.Optional<String> permissionSubject(I input, ToolContext ctx) { return java.util.Optional.empty(); }

    /** 입력 JSON Schema. record로부터 자동 생성(8장). */
    default JsonNode inputSchema() { return agent.tool.schema.JsonSchemas.forRecord(inputType()); }
}
