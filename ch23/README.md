...
[`chapters/23-서브에이전트와-Task.md`](../../chapters/23-서브에이전트와-Task.md)의 산출물. 메인 에이전트가 하위 작업을 격리된 서브에이전트에 위임하는 **`AgentTool`**(와이어 이름 `Agent`)을 만든다. 핵심은 12장 `AgentEngine`을 **재귀적으로 재사용**한다는 점이다 — 서브에이전트는 새 엔진(=새 메시지 배열)으로 돌고, 부모는 그 *결과(요약)만* `tool_result`로 받아 컨텍스트를 아낀다.
