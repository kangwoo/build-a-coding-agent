...
책 15장의 산출물. 가장 강력하고 위험한 도구인 **`BashTool`** — *아무* 셸 명령이나 실행하고 합쳐진 출력(merged fd)을 캡처한다. 11장 외부 프로세스 패턴(`RipGrep`: 가상 스레드 드레인·타임아웃) 위에 14장 취소(`onCancel`/`isInterrupt`)를 얹어, 프로세스 수명주기(타임아웃·강제 종료·tree-kill·종료코드 해석)를 정밀하게 다룬다. 권한은 16장에서 붙고, 지금은 무확인 실행이다.
