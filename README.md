...
책 [**《밑바닥부터 만드는 코딩 에이전트 — 자바 21로 구현하는 LLM 에이전트의 내부》**](https://product.kyobobook.co.kr/detail/S000220871868)의 챕터별 예제 코드 저장소입니다.
...
> 챕터가 진행되며 코드는 멀티모듈(`agent-core` + `agent-cli`)로 자랍니다(정식 골격은 4장·부록 A). 이후 장의 `chXX/`는 직전 장의 산출물 위에 누적됩니다. 즉 ch11은 ch10에 그 장의 추가분을 얹은 것입니다. 다만 임시 코드는 역할을 다하면 사라집니다 — 예컨대 4장의 `Wire` 어댑터는 5장에서 삭제됩니다(해당 장 README에 표시).
