...
[`chapters/13-도구-오케스트레이션과-스트리밍.md`](../../chapters/13-도구-오케스트레이션과-스트리밍.md)의 산출물. 12장 루프는 한 턴의 여러 `tool_use`를 하나씩 직렬로 실행했다. 이 장은 그 ④단계를 `ToolOrchestrator`로 교체해 "안전한 건 병렬, 위험한 건 단독 직렬"로 분배하되, 결과는 **모델이 부른 순서 그대로** 돌려준다. 병렬로 도구가 돌기 시작하므로, 14장의 계층적 취소 트리가 이 실행 위에 올라간다.
