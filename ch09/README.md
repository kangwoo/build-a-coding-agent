...
[`chapters/09-파일-읽기-도구와-멀티모달.md`](../../chapters/09-파일-읽기-도구와-멀티모달.md)의 산출물. 첫 번째 진짜 도구인 **`ReadTool`**(라인 번호·범위 읽기·바이트 한도·중복 회피)과, "수정 전 반드시 Read 선행" 규칙(10장)을 받쳐 줄 **`FileStateCache`**를 만든다. 출력은 파일 종류별로 갈래지므로 `ReadResult` sealed 타입으로 표현한다.
