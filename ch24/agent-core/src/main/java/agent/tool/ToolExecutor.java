...
public final class ToolExecutor {
    ...
    /** 게이트 없이(allow-all). 테스트·신뢰 환경용 3-arg 오버로드. */
    public static ToolResultBlock runToolUse(Tool<?, ?> tool, ToolUseBlock use, ToolContext ctx) {
        return runToolUse(tool, use, ctx, PermissionGate.allowAll());
    }
     ...
     */
    public static ToolResultBlock runToolUse(Tool<?, ?> tool, ToolUseBlock use,
                                             ToolContext ctx, PermissionGate gate) {
        return exec(tool, use, ctx, gate);   // 와일드카드 캡처용 위임
    }

    private static <I, O> ToolResultBlock exec(Tool<I, O> tool, ToolUseBlock use,
                                               ToolContext ctx, PermissionGate gate) {
        ...
        try {
            ...
            // ④ 권한 게이트(규칙+모드+도구검사+대화형 ask 해소). Deny면 거부 사유를 모델에 돌려준다.
            if (gate.decide(tool, input, ctx) instanceof PermissionResult.Deny d) {
                return error(use.id(), d.message());
            }
