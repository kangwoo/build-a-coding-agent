...
public final class AgentEngine {
    ...
    private void runLoop(EventStreams.Sink<AgentEvent> sink, CancellationToken cancel) {
        ...
        while (true) {
            ...
            // ④ 도구 실행 → tool_result (불변식: 모든 tool_use에 결과)
            List<ContentBlock> results = new ArrayList<>();
            for (ToolUseBlock use : toolUses) {
                sink.emit(new AgentEvent.ToolStarted(use.name(), use.input()));
                ToolResultBlock result = tools.find(use.name())
                        .map(t -> ToolExecutor.runToolUse(t, use, ctx(cancel)))
                        .orElse(ToolResultBlock.error(use.id(), "알 수 없는 도구: " + use.name()));
                sink.emit(new AgentEvent.ToolFinished(use.name(), result.isError()));
                results.add(result);
            }

            // ⑤ tool_result를 user 메시지로 붙이고 다음 턴
            messages.add(Message.UserMessage.ofBlocks(results));
        }
    }

    private ToolContext ctx(CancellationToken cancel) {
        return new ToolContext(java.nio.file.Path.of("").toAbsolutePath(), cancel, fileState);
    }
